(function ($) {
	...
	var yesCount = parseInt(getCookie('yesCount')) || 0;
	var noCount = parseInt(getCookie('noCount')) || 0;
  
	// Update the count displays
	function updateCounts() {
	  $('#yesCount').text('Total Yes: ' + yesCount);
	  $('#noCount').text('Total No: ' + noCount);
	}
	...
	$('.feedback-btn-wrap .feedback-btn').click(function () {
		...
		if (existingCookie === value) {
			...
			if (value === 'yes') {
			  ...
			  // if clcick yes minus no
			  // if click no minus yes
			...
			var oppositeValue = value === 'yes' ? 'no' : 'yes';
			...
			if (value === 'yes') {
			  ...
			  if (oppositeCookie) {
				// Remove the opposite button's cookie
				removeCookie('feedback_' + dataId);
    ...
    // Update and store the counts in cookies
	...
	$.ajax({
		...
		success: function (response) {
			...
			$('#yesCount').text('Total Yes: ' + yesCount);
			$('#noCount').text('Total No: ' + response);
