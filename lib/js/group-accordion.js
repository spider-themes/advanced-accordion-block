...
(function ($) {
	...
	// if ($('.wp-block-aab-group-accordion.autoplay').length) {
	// 	let interval = 10000; // Autoplay interval in milliseconds

	// 	const autoplayAccordions = () => {
	// 		const groupAccordions = $('.wp-block-aab-group-accordion.autoplay');
	// 		groupAccordions.each(function () {
	// 			const accordions = $(this).find('.aagb__accordion_container');

	// 			let currentIndex = 0;

	// 			const triggerAccordionHover = (index) => {
	// 				accordions
	// 					.eq(index)
	...
	// 				// Reset the previous accordion
	// 				triggerAccordionHover(currentIndex);

	// 				// Move to the next accordion
	// 				currentIndex = (currentIndex + 1) % accordions.length;

	// 				// Trigger hover on the next accordion
	// 				triggerAccordionHover(currentIndex);
	// 			};
	...
	// 			// Start autoplay
	// 			let autoplayInterval = setInterval(cycleAccordions, interval);

	// 			// Stop autoplay on hover
	// 			accordions.on('mouseenter', () =>
	// 				clearInterval(autoplayInterval)
	// 			);
	// 			accordions.on('mouseleave', () => {
	// 				autoplayInterval = setInterval(cycleAccordions, interval);
	// 			});
	// 		});
	// 	};
	...
	$(document).ready(function () {
		...
		function transitionSlide() {
			...
			$('.wp-block-aab-group-accordion.autoplay .panel')
				.eq($item)
				.addClass('aagb__accordion_active');

			$(
				'.wp-block-aab-group-accordion.autoplay .panel.aagb__accordion_active'
			)
				.find('.aagb__accordion_body')
			...
			if (
				$('.wp-block-aab-group-accordion.autoplay .panel').hasClass(
					'aagb__accordion_active'
				)
			) {
				if ($icon.hasClass('dashicons-plus-alt2')) {
				...
				if ($(this).hasClass('dashicons-minus')) {
					...
					$(this).removeClass('dashicons-arrow-up');
					$(this).addClass('dashicons-arrow-down');
				} else if ($(this).hasClass('dashicons-arrow-up-alt2')) {
					$(this).removeClass('dashicons-arrow-up-alt2');
					$(this).addClass('dashicons-arrow-down-alt2');
				} else if ($(this).hasClass('dashicons-dismiss')) {
					$(this).removeClass('dashicons-dismiss');
